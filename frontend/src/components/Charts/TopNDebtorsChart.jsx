...
export default function TopNDebtorsChart() {
  ...
  for (const row of debtData) {
    if (row.country_name && row.value !== null) {
      countryDebtMap[row.country_name] = (countryDebtMap[row.country_name] || 0) + row.value;
    }
  }
    ...
    <Box sx={{ width: "100%", height: 450 }}>
      ...
      <ResponsiveContainer width="100%" height="120%">
        ...
        >
          ...
          <YAxis type="category" dataKey="country" width={120}  interval={0}
            tickFormatter={(value) => value.length > 10 ? `${value.substring(0, 10)}...` : value} />
          <Tooltip formatter={(value) => value.toLocaleString()} />
          <Bar dataKey="total" fill="#1976d2">
            <LabelList dataKey="total" position="right" formatter={(value) => `${(value / 1e9).toFixed(1)}B`} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </Box>
